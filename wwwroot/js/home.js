...
const createTrTable = function (index, product) {
   ...
   <td class="subtotal text-center">${convertVND(subtotal * 1000)}</td>`;
   return trElement;
};
...
const removeFromCart = function (button) {
   ...
   const existingProduct = cart.find((item) => item.id === button.dataset.id);
...
const updateTotal = function () {
   ...
   document.querySelector("#cart-total").textContent = `Total: ${convertVND(total * 1000)}`;
};
